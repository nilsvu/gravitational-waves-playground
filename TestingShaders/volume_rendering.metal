...
float3 cartesianToPolar(float3 x) {
    return float3(length(x), fast::atan2(length(x.xz), x.y) + M_PI_F, fast::atan2(-x.z, x.x) + M_PI_F);
                            ...
                            texture2d<float, access::sample> colorSampler [[texture(0)]])
{
    ...
    if (maximumRayLength > 0.0 && t < 0.0) {
        ...
        // TODO: make sure occlusion works, bug that also field in front of sphere is occluded?
    }
    ...
    while (integratedOpacity < 1.0 && rayLength < maximumRayLength) {
        ...
        float3 x = rayPosition - sourcePosition;
        float3 xPolar;
        float3 xz = float3(x.x, 0.0, x.z);
        float l = length(xz);//sqrt(pow(x.x, 2.0) + pow(x.z, 2.0));
        xPolar.x = length(x);
        xPolar.y = fast::atan2(l, x.y) + M_PI_F;
        xPolar.z = fast::atan2(-x.z, x.x) + M_PI_F;
